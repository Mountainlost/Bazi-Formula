...
def test_audit_rejects_career_event_promise(tmp_path: Path) -> None:
    ...
    rules["career_reading_v0"]["conclusion"] += " 一定升职。"
    ...
    assert checks["career_reading_not_absolute_verdict"].passed is False


def test_audit_rejects_career_shen_sha_overreach(tmp_path: Path) -> None:
    chart = _load_chart(tmp_path)
    rules = build_rules_output(chart)
    report = build_report(chart, rules).model_dump()

    report["final_report_v0"]["career"]["text"] += " 天乙贵人定事业。"
    ...
    checks = {item.name: item for item in audit.checks}
    assert audit.passed is False
    assert checks["final_report_career_not_shen_sha_verdict"].passed is False


def test_audit_rejects_career_annual_flow_overreach(tmp_path: Path) -> None:
    chart = _load_chart(tmp_path)
    rules = build_rules_output(chart)
    report = build_report(chart, rules).model_dump()

    report["final_report_v0"]["career"]["text"] += " 流年骨架定事业。"
    ...
    checks = {item.name: item for item in audit.checks}
    assert audit.passed is False
    assert checks["final_report_career_not_annual_flow_verdict"].passed is False
